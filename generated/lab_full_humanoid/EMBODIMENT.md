---
name: lab_full_humanoid
description: 'Embodiment context for 5 device(s). Active profile: full_humanoid. Switch
...
Active profile: **`full_humanoid`** — Full humanoid capability. Motion requires confirmation. Manipulation requires confirmation plus supervisor signoff. Audio output requires confirmation. Nothing is absolutely forbidden at the framework level; task-level readiness checks govern execution.

| class | behavior under this profile |
|---|---|
| `safe` | plan freely |
| `motion` | requires confirmation |
| `manipulation` | requires supervisor |
| `audio_output` | requires confirmation |
...
- **`full_humanoid`**: All capabilities available. Motion requires confirmation. Manipulation requires confirmation + supervisor. ← current
...
- *User*: "Walk forward 1 meter."
  *Plan*: ask user to confirm ("Confirm: G1 will walk 1 m forward. Proceed?"). If confirmed, plan `walk` with `g1_real_sensor_only`; run obstacle_check and floor_clear readiness first.

- *User*: "Pick up the cup."
  *Plan*: require user confirmation AND supervisor signoff. State: "This requires supervisor sign-off in addition to your confirmation." If both obtained, plan `grasp`+`pick` with `g1_real_sensor_only` after object_identified and grasp_plan_verified readiness checks.
