---
name: lab_motion_supervised
description: 'Embodiment context for 5 device(s). Active profile: motion_supervised.
  Switch profile or constrain via prompt to change allowed actions.'
version: 0.3.0
kind: embodiment_context
active_safety_profile: motion_supervised
primary_graph: observation
---

# Lab Motion Supervised
...
Active profile: **`motion_supervised`** — Locomotion enabled with per-command user confirmation. Manipulation still requires a supervisor. Audio output requires confirmation.
...
- **`motion_supervised`**: Locomotion enabled with confirmation. Manipulation forbidden. Good for mobile inspection. ← current
...
- *User*: "Pick up the cup."
  *Plan*: **refuse** — `pick` is forbidden under the `motion_supervised` profile. Tell the user to switch to `full_humanoid` profile (which requires supervisor signoff) and try again.
